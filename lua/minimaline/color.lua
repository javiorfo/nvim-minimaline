...
local colors = {
        ...
        "#00D787", "#00D7AF", "#00D7D7", "#00D7FF", "#00FF00", "#00FF57", "#00FF87",
...
-- Function to get hexadecimal color from xterm number
-- @param xterm_nr. Ex: 14
-- @return hexadecimal color
local function get_hex_color(xterm_nr)
    if xterm_nr > 255 or xterm_nr < 0 then
        return nil
    end
    return xterm_nr, colors[xterm_nr + 1]
end

-- Function to hightlight specific group 
-- @param group. Ex: "Statement" 
-- @param options. Ex: { fg = 15, bg = 0, gui = "bold" } 
function M.highlight(group, options)
    if options == nil or next(options) == nil then return end

    local cterm_fg = options.fg or "NONE"
    local cterm_bg = options.bg or "NONE"

    local fg = "NONE"
    if cterm_fg ~= "NONE" then
        fg = select(2, get_hex_color(options.fg))
    end

    local bg = "NONE"
    if cterm_bg ~= "NONE" then
        bg = select(2, get_hex_color(options.bg))
    end

    local gui = options.gui
    if gui then
        gui = string.format(" gui=%s cterm=%s", gui, gui)
    else
        gui = ""
    end

    local hi = string.format("hi %s ctermfg=%s ctermbg=%s guifg=%s guibg=%s %s", group, cterm_fg, cterm_bg, fg, bg, gui)
    vim.cmd(hi)
end
